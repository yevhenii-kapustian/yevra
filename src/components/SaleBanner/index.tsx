...
export default async function SaleBanner() {
  const saleProducts = await getProductsByCategory("sale");
  if (saleProducts.length === 0) return null;

  const maxDiscountPct = Math.max(...saleProducts.map((p) => p.discountPct));
  ...
  return (
    <section className="mx-4 sm:mx-8 lg:mx-12 mb-14 bg-accent rounded-md p-7 sm:p-14 flex items-center justify-between gap-5 flex-wrap">
      <div>
        <div className="text-2xl font-extrabold text-white">Season Sale — up to {maxDiscountPct}% off</div>
